...
// ============================================================================
// CONSTRAINT TYPES (same as before)
// ============================================================================

export type CType =
  | { kind: "COUNT_EQ"; mask: number; wantCrim: boolean; k: number }
  ...
  | { kind: "NAME_STATUS_IN_GROUP"; nameIdx: number; mask: number; wantCrim: boolean; k?: number }
  | { kind: "INTERSECTION_EMPTY"; maskA: number; maskB: number; wantCrim: boolean }
  | { kind: "UNIQUE_ROW_COUNT_EQ"; rowMasks: number[]; wantCrim: boolean; numRows: number; countPerRow: number }
  ;
...
type TokenType = "LPAREN" | "RPAREN" | "IDENT" | "NUMBER" | "OP" | "EOF";
...
interface SetExpr {
  type: "primitive" | "intersection" | "union" | "complement" | "filtered";
  mask?: number;
  role?: "criminal" | "innocent" | null; // null = unfiltered
  left?: SetExpr;
  right?: SetExpr;
}
...
class DSLParser {
  ...
  constructor(tokens: Token[], board: BoardSnapshot) {
    this.tokens = tokens;
    this.pos = 0;
    this.board = board;
  }
  ...
  nameToIndex(name: string): number | null {
    const t = name.toLowerCase();
    for (const c of this.board.cells) {
      if (c.name && c.name.toLowerCase() === t) return c.id;
    }
    return null;
  }

  // Parse primitives: area(row 1), neighbor(Gary), above(Uma), etc., or (expr)
  parsePrimitive(): SetExpr {
    const tok = this.peek();
    ...
    switch (func) {
      ...
      case "neighbor": {
        ...
        if (idx === null) throw new Error(`Unknown name: ${nameToken.value}`);
      ...
      case "above": {
        ...
        if (!cell.pos) throw new Error(`No position for ${nameToken.value}`);
      ...
      case "between": {
        ...
        if (idx1 === null) throw new Error(`Unknown name: ${name1Token.value}`);
        if (idx2 === null) throw new Error(`Unknown name: ${name2Token.value}`);
        
        const cell1 = this.board.cells[idx1];
        const cell2 = this.board.cells[idx2];
        if (!cell1.pos || !cell2.pos) throw new Error(`No position for cells`);
  ...
  evalSetExpr(expr: SetExpr): { mask: number; role: "criminal" | "innocent" | null } {
    switch (expr.type) {
      case "primitive":
      ...
      default:
        throw new Error(`Unknown set expression type: ${(expr as any).type}`);
    }
  }

  // Parse entire DSL expression
  parseExpression(): SetExpr {
...
function compileDSL(dslInput: string, board: BoardSnapshot): CType[] {
  ...
  for (const line of lines) {
    ...
    if (func === "eq") {
      ...
      let rightRole: string | null = null;
