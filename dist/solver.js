...
function evalConstraint(cons, C) {
    switch (cons.kind) {
        case "COUNT_EQ": {
            const bits = cons.wantCrim ? (C & cons.mask) : (~C & board_1.FULL_MASK & cons.mask);
        ...
        case "UNIQUE_COUNT_EQ": {
            ...
            for (const gm of cons.groupMasks) {
                const bits = cons.wantCrim ? (C & gm) : (~C & board_1.FULL_MASK & gm);
        ...
        case "NEIGHBOR_COUNT": {
            if (cons.qualifier === "IN_ROW" && cons.qualifierMask) {
                const bits = cons.wantCrim ? (C & cons.mask & cons.qualifierMask) : (~C & board_1.FULL_MASK & cons.mask & cons.qualifierMask);
                return bitCount(bits) === cons.k;
            }
            return false;
        }
        case "CONN_ALL": {
            ...
            const bits = cons.wantCrim ? (C & cons.mask) : (~C & board_1.FULL_MASK & cons.mask);
            if (bits === 0)
                return false; // per tutorial, "All ..." means at least one exists
        ...
        case "INTERSECTION_EMPTY": {
            const aBits = cons.wantCrim ? (C & cons.maskA) : (~C & board_1.FULL_MASK & cons.maskA);
            const bBits = cons.wantCrim ? (C & cons.maskB) : (~C & board_1.FULL_MASK & cons.maskB);
            return (aBits & bBits) === 0;
        }
        case "UNIQUE_ROW_COUNT_EQ": {
            let hits = 0;
            for (const rowMask of cons.rowMasks) {
                const bits = cons.wantCrim ? (C & rowMask) : (~C & board_1.FULL_MASK & rowMask);
                if (bitCount(bits) === cons.countPerRow)
                    hits++;
                if (hits > cons.numRows)
                    return false; // early exit if too many
            }
...
function solve(board) {
    ...
    function enumerateSolutions(b) {
        ...
        const unknownMask = (~knownMask) & board_1.FULL_MASK;
        const constraints = (0, parser_rules_1.compileClues)(b.clues, b);
        const unknownIndices = [];
        for (let i = 0; i < board_1.N_CELLS; i++)
    ...
    for (let idx = 0; idx < board_1.N_CELLS; idx++) {
...
exports.solve = solve;
exports.default = { solve, compileClues: parser_rules_1.compileClues };
